...
- `PLUGIN_ARCHITECTURE.md` v1.0.0 (1107 рядків)
- `PLUGIN_CONTRACT.md` v1.0.0 (555 рядків)
- `PLUGIN_INTERFACES_EXTENDED.md` v1.1.0 (464 рядки)
- `PLUGIN_DIAGNOSTICS.md` v1.0.0 (683 рядки)
- `CREATING_PLUGINS.md` (607 рядків)
...
|---|---|---|---|---|
| **PA-1** | ARCH §2, INTERFACES §1, CREATING | `SensorData` 4 vs 5 полів — два несумісних визначення | 🔴 CRITICAL | 🔓 OPEN |
| **PA-2** | ARCH §"Внутрішня структура" | `initializeAll()` викликає `plugin->initialize()` без `PluginContext*` | 🔴 CRITICAL | 🔓 OPEN |
| **PA-3** | ARCH §2, DIAGNOSTICS §1 | `IPlugin` vs `IDiagnosticPlugin`: два документи визначають несумісні ієрархії | 🔴 CRITICAL | 🔓 OPEN |
| **PA-4** | DIAGNOSTICS §2 | LDC1101 `readRP()`: MSB першим — hardware data corruption | 🟠 HIGH | 🔓 OPEN |
| **PA-5** | ARCH §"Thread Safety", CONTRACT §2.2, CREATING §"Async" | `portMAX_DELAY` у 5 місцях — порушує ADR-ST-008 | 🟠 HIGH | 🔓 OPEN |
| **PA-6** | ARCH §Memory, CONTRACT §2.4 | Heap budget 512 KB / PSRAM недосяжні на ESP32-S3FN8 без PSRAM | 🟠 HIGH | 🔓 OPEN |
| **PA-7** | ARCH §"Внутрішня структура" | `dynamic_cast` без гарантії RTTI (`-fno-rtti` default) | 🟠 HIGH | 🔓 OPEN |
| **PA-8** | DIAGNOSTICS §4 | `pluginSystem->getContext(plugin)` — метод не існує ніде | 🟠 HIGH | 🔓 OPEN |
| **PA-9** | DIAGNOSTICS §2 | `runSelfTest()` → `Serial.println()` — порушує CONTRACT §2.4 | 🟡 MEDIUM | 🔓 OPEN |
| **PA-10** | ARCH §PluginContext, CREATING §"Конфіг" | `ConfigManager` API (getInt, getUInt8, getFloat...) ніде не визначено | 🟡 MEDIUM | 🔓 OPEN |
| **PA-11** | DIAGNOSTICS §2 | `writeRegister(LDC1101_CONFIG, 0x15)` — значення 0x15 не існує в register map | 🟡 MEDIUM | 🔓 OPEN |
...
| **PA-14** | INTERFACES §5 | `IStoragePlugin` vs `STORAGE_ARCHITECTURE` standalone — межа не визначена | 🟡 MEDIUM | 🔓 OPEN |
| **PA-15** | CONTRACT §2.1 | `calibrate()` може блокуватись (delay 2–5 сек) — не задокументовано в CONTRACT | 🟡 MEDIUM | 🔓 OPEN |
| **PA-16** | CREATING §"Складні випадки" | Plugin dependency injection — механізм не визначено | 🟢 LOW | 🔓 OPEN |
| **PA-17** | DIAGNOSTICS §2 | `millis() - lastCalibrationTime > 30 days` — millis() overflow + non-persistent | 🟢 LOW | 🔓 OPEN |
| **PA-18** | DIAGNOSTICS §"Додаткові фічі" | `WiFiClient HTTP POST` суперечить Connectivity Architecture (BLE+WS) | 🟢 LOW | 🔓 OPEN |
