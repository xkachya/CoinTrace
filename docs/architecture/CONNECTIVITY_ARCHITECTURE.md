...
Characteristics:
  ...
  RESULT    UUID: 9AF88885-31CE-412B-A567-94052E9598BC  Properties: Notify
                  ...
                  conf       = uint8 0–100 (%)
                  rp/k1/k2/slope/dl = packed floats (роздільна здатність: Rp 0.1Ω, k 0.001, slope 0.001/mm, L 0.01µH)
...
> **Canonical формат** — синхронізований з `FINGERPRINT_DB_ARCHITECTURE.md` v1.4.0. **Будь-який транспорт (Serial, HTTP, WebSocket) використовує той самий named-object vector.** Масив як вектор заборонений скрізь — порядок елементів стає частиною формату, одна транспозиція = тихо неправильний результат.
...
```json
{
  ...
  "raw": {
    "rp0": 42180, "rp1": 41867, "rp2": 41550, "rp3": 41200,
    "l0":  31450, "l1":  31268
  },
  "match": {
    "name":       "Silver 925",
...
**Причина:** WiFi + BLE + AsyncWebServer + Logger buffers одночасно → heap ~180–200 KB зайнято з 337 KB. з урахуванням `index.json` в RAM (~80 KB для 1000 монет) — залишки ~55–65 KB, які вже на межі OOM (детальний аналіз — §8.7).  
...
| Компонент | RAM (типово) |
|---|
| WiFi stack | ~70 KB |
| BLE stack | ~60 KB |
| AsyncWebServer + 2 з'єднань | ~25–40 KB |
| Logger RingBuffer (100 записів) | ~10 KB |
| **index.json в RAM (1000 монет)** | **~80 KB** |
| ArduinoJson parsing overhead | ~4 KB |
| FreeRTOS tasks overhead | ~8 KB |
| **РАЗОМ** | **~257–272 KB** |

Залишок: `337 - 272 = ~65 KB` — це **до** fingerprint identification (distance calculation, `_aggregate.json` reads).

**Висновок:** WiFi + BLE + index.json (1000 монет) одночасно = на межі OOM. 
...
Стан WiFi не впливає на стан WiFi (AP ↔ STA без reboot)
```
