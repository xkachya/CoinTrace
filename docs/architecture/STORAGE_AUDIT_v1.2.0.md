...
#### [PRE-1] 🔴 Вирішити LittleFS Space Budget (M1-C1)

**Дія:** Прийняти одне з рішень:

- **Рекомендовано:** Варіант A — скоротити log rotation до **2 файли × 200 KB** (замість 3×300 KB). Оновити §12.1, OQ-07, §13.1.
- Або: Варіант D — збільшити `littlefs_data` partition до `0x220000` (2.125 MB) у `cointrace_8MB.csv`. Потребує зменшення coredump.

**Верифікація:** Після рішення — повторити симуляцію блоків (або виміряти `lfs_info()` на реальному пристрої після завантаження тестового набору файлів).

**Документ для оновлення:** §12.1 (log sizes), §13.1 (wear calc), §6 (partition CSV якщо Варіант D).
...
**Статус документа STORAGE_ARCHITECTURE v1.2.0:** архітектурно зрілий, добре обґрунтований. Один критичний блокер (LittleFS space budget) потребує вирішення та закриття кількох HIGH-знахідок перед переходом до фази P-3.

---

*Аудит виконано: 2026-03-12 · Версія аудиту: 1.0 · Наступний аудит: після вирішення [PRE-1]..[PRE-8]*
