...
**5. CLKIN — потрібний тільки для LHR режиму (ADR-LHR-001).** LDC1101 має внутрішній LC-осцилятор: **RP+L режим (16-bit, активний за замовчуванням) не потребує зовнішнього CLKIN**. CLKIN задіяний виключно в LHR (24-bit) режимі — для точного розрахунку fSENSOR через підрахунок тактів за RCOUNT-вікно (Eq.11, datasheet §8.4.3).

> **Апаратні піни (MIKROE-3240 / mikroBUS):**
> - **CLKIN — mikroBUS Pin 16 (PWM):** вхід зовнішнього clock. При LHR: подати 16 MHz від ESP32-S3 (LEDC timer або sigma-delta). Для RP+L: лишити не підключеним — дивись `docs/hardware/LDC1101_WIRING.md` §4.
> - **CLDO — вихід chip-level clock з LDC1101:** не виведений на mikroBUS у MIKROE-3240, не задіяний у поточній архітектурі. Може слугувати джерелом clock для MCU при LHR якщо виведений на кастомній платі.
>
> ⚠️ **Для реалізації ADR-LHR-001** потрібен дротовий зв'язок GPIO → mikroBUS Pin 16 та конфігурація LEDC timer ESP32-S3 на 16 MHz. Без цього підключення LHR підсистема не отримає reference clock і дасть некоректні значення fSENSOR. Backlog: §10 задача 10.
...
| `clkin_freq_hz` | uint32 | 16000000 | Частота зовнішнього CLKIN. **Використовується тільки в LHR режимі** (Eq.11, ADR-LHR-001) — RP+L не потребує CLKIN (чіп використовує внутрішній LC-осцилятор). Значення 16 MHz відповідає рекомендованому LEDC clock ESP32-S3 при LHR. Eq.6 (RP+L fSENSOR) CLKIN не використовує. |
