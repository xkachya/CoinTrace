...
**5. CLKIN — потрібний для будь-якого вимірювання L (і RP+L, і LHR).** ⚠️ **ADR-CLKIN-002 (2026-03-23, виправлення):** Попередня версія цього пункту стверджувала, що RP+L режим не потребує CLKIN — це **помилково**. Datasheet SNOSD01D §9.1.8 прямо вказує: *"The L and LHR measurements will return 0"* без CLKIN. Без стабільного clock на CLKIN пін L_DATA = 0 (якщо CLKIN притягнутий до GND) або нестабільний garbage (якщо CLKIN плаваючий).

| Вимірювання | Потребує CLKIN? | Без CLKIN |
|---|---|---|
| **RP_DATA** | ❌ Не потребує | ✅ Коректний (amplitude-based) |
| **L_DATA (RP+L 16-bit, Eq.6)** | ✅ Так | 0 або garbage |
| **LHR_DATA (24-bit, Eq.11)** | ✅ Так | 0 або garbage |

RP вимірюється через кількість енергії для підтримки амплітуди LC-осцилятора — повністю внутрішній процес. L вимірюється як frequency ratio fSENSOR/fCLKIN — без reference clock (fCLKIN) відношення невизначене.

> **Апаратні піни (MIKROE-3240 / mikroBUS):**
> - **CLKIN — mikroBUS Pin 16 (PWM):** вхід зовнішнього clock. Для RP+L **і** LHR: подати 16 MHz від ESP32-S3 (LEDC timer, G4). Без CLKIN: притягнути до GND для стабільного L=0. Дивись `docs/hardware/LDC1101_WIRING.md` §3.
> - **CLDO — вихід chip-level clock з LDC1101:** не виведений на mikroBUS у MIKROE-3240, не задіяний у поточній архітектурі.
>
> ⚠️ **Для коректних L вимірювань** потрібен дротовий зв'язок GPIO (G4) → mikroBUS Pin 16 та конфігурація LEDC timer ESP32-S3 на 16 MHz. Без цього L_DATA некоректний в обох режимах. RP_DATA працює коректно без CLKIN. Backlog: §10 задача 10.
...
> Protocol p3 використовує d₀=0.6mm (3D-друк, bare coin) як практичний мінімум. Для чисто феромагнітних монет (Ni, сталь)
> ADR-SPACER-001 1.5mm залишається актуальним.
