...
**4. RP_SET — критичний для осциляції.** Регістр `0x01` визначає динамічний діапазон RP вимірювання. Якщо котушка не вкладається в заданий діапазон — `STATUS_NO_OSC` спрацює і жодних вимірювань не буде. Default значення `0x07` (найширший діапазон) коректне для більшості котушок нумізматики.
...
Для котушок 1–5 MHz (типовий діапазон нумізматики) навіть `RESP_TIME = 0x07` дає конверсію < 1 мс — значно менше 20 мс між викликами `update()` при 50 Hz. Використовувати `0x07` як стандартне значення.
...
| `rp_set` | uint8 | 7 | RP_SET регістр (0x01). Визначає динамічний діапазон RP. Default 7 = найширший. Змінювати при `NO_SENSOR_OSC` якщо котушка нестандартна. |
...
public:
    ...
    bool initialize(PluginContext* context) override {
        ...
        if (ctx->config) {
            ...
            rpSetValue    = ctx->config->getUInt8("ldc1101.rp_set", 0x07);
...
private:
    ...
    bool configureSensor() {
        ...
        // RP_SET: динамічний діапазон. Default 0x07 = найширший діапазон.
        // Змінювати якщо STATUS_NO_OSC при нестандартній котушці.
        ...
        // DIG_CONFIG: RESP_TIME bits[2:0], MIN_FREQ bits[7:4] = 0 (default 500 kHz min)
        uint8_t digCfg = respTimeBits & 0x07;
