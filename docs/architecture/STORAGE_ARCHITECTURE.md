...
│   │                               # Недійсний якщо SD DB оновилась (перевіряємо CRC32)
...
Block budget (LittleFS, block_size=4KB, inline_max=256B):
  ...
  cache/        : index.json 80KB = 21 blocks + index_crc32.bin 4B (inline) = 84 KB
...
[7] SDCardManager::tryMount()                ; SD — optional, failure = DEGRADED
    ...
    a) SD є, cache `/data/cache/index.json` є → перевірка CRC32 (index_crc32.bin); якщо не збігається → видалити cache і index_crc32.bin
