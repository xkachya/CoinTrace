...
**Статус:** ✅ Accepted v1.2.0 — shared VSPI bus підтверджено схемою P3, критичні знахідки адресовані
**Версія:** 1.2.0
**Дата:** 2026-03-12
...
**Рішення:** 3 ротації × 300 KB = 900 KB hot storage в LittleFS_data.

- **LittleFSTransport** = live logs у `log.0.jsonl`; при > 300 KB → ротація (.0→.1→.2, delete .2).
- **SDTransport** = cold archive: при ротації `log.2.jsonl` копіюється на SD (batch, не per-entry).
...
> **✅ OQ-07 закрито (архівна семантика SD):**
> SDTransport отримує log-записи **тільки при ротації LittleFS** (копія `log.2.jsonl` → SD). записи з SD 
> **не є** live-дублікатом LittleFSTransport (не отримує кожен LIT вхід). LittleFS = «гарячі»
> (recent 900 KB), SD = «холодний архів» (повні ротації). 3 ротації × 300 KB = 900 KB total hot storage.
...
`LittleFS_data = 1.75 MB = 448 секторів (1792 KB ÷ 4 KB)`. При 72 writes/добу по wear-leveled filesystem:
```
...
[1.5] GPIO0 recovery check                  ; BOOT button (active LOW, підтяжка 10kΩ)
    if GPIO0 == LOW && millis() < 3000:
        display("BOOT HELD — Force format? Release to cancel")
        delay(5000)
...
[7.5] esp_core_dump_image_check()            ; P-05: ПІСЛЯ mount SD — coredump може зберегтись
    → якщо є coredump:
      якщо SD змонтована → SD:/CoinTrace/coredumps/<ts>.json
      якщо SD відсутня  → /data/logs/coredump_last.json (summary < 4 KB)
...
| Coredump detected | — | Зберегти: SD (пріоритет) → LittleFS /data/logs/coredump_last.json → Serial. Очистити partition, LOG_WARN, продовжити boot. |
