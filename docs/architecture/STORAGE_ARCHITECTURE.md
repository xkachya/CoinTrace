...
**Статус:** ✅ Accepted v1.0.0 — всі архітектурні рішення зафіксовані, готово до реалізації
**Версія:** 1.0.0
**Дата:** 2026-03-12
...
> **Прийняте рішення: Варіант A з mutex** для першої версії (менше змін у hardware wiring).
> Варіант B — якщо профілювання покаже що SD mutex є bottleneck.
>
> ⚠️ **Критично важлива: скоп очікування mutex** (R-04)
>
> **mutex береться на один SPI-барст (одне вимірювання дистанції), не на весь 4-дистанційний цикл.**
>
> Обгрунтування:
> - Вимірювання 4 дистанцій — Ѯсер фізично переміщує дистанціюр між вимірами (5–30 секунд пауза). Протягом цієї паузи SPI-шина вільна.
> - SPI-барст LDC (10–500 мс) відбувається тільки протягом циклу безпосереднього зчитування Rp/L (rp0, l0 для однієї дистанції).
> - `slope_rp_per_mm_lr` обчислюється з просторових позицій (0, 1, 3 мм), а не з часових інтервалів → пауза між барстами не впливає на якість вектору.
> - Блокування SD на 20–120 секунд (mutex на весь цикл) спричинило б переповнення черги Logger (silent data loss).
>
> **Правильна реалізація:**
> ```cpp
> // LDC запитує mutex перед кожним SPI-барстом, звільняє після
> for (int dist = 0; dist < 4; dist++) {
>     xSemaphoreTake(spi_mutex, portMAX_DELAY);  // зачекає тільки під час SPI burst
>     ldc.readRpL(&rp[dist], &l[dist]);          // ~10–500 мс
>     xSemaphoreGive(spi_mutex);                 // звільняє: SD сам може писати
>     // ПАУЗА: користувач переміщує дистанціюр (5–30 с)
> }
> ```
...
| `SDTransport::write()` | ✅ Так | FreeRTOS queue, SPI mutex per burst |
...
|---|---|
| `GET /api/v1/status` | NVS read: heap, version, bat, wifi, ble + **LittleFS/SD stats** (free/total bytes). Потребує синхронізації з CONNECTIVITY_ARCHITECTURE v1.2.0. |
...
| **OQ-03** | ~~Ring buffer size (50 вимірів достатньо?)~~ | ~~UX, storage allocation~~ | ✅ **Вирішено: 300** |
| **OQ-04** | Де plugin config.json? | Boot dependency | Архітектурне рішення |
| **OQ-05** | ~~NVS flash encryption до v1 чи після?~~ | ~~Security, one-way decision!~~ | ✅ **Вирішено: без encryption v1** |
...
*Версія 1.0.0 — Архітектурні рішення зафіксовані, готово до реалізації*
*Наступний крок: вирішити OQ-01, OQ-02, OQ-04, реалізувати P-1 (partition table + NVSManager)*
...
[7] SDCardManager::tryMount()                ; SD — optional, failure = DEGRADED
    → якщо SD є: валідація index.json cache (hash check, A-02)
      якщо cache invalid: invalidate /data/cache/index.json
