...
**Статус:** 📝 Draft v0.1.0 — готовий до обговорення, не імплементовано
**Версія:** 0.1.0
**Дата:** 2026-03-12
...
M5Stack Cardputer-Adv має microTF slot підключений через **SPI**:

```
SD SPI pins (M5Stack Cardputer-Adv, потребує верифікації на фізичному пристрої):
  MOSI: GPIO 14  (ймовірно, спільний з SPI Bus)
  MISO: GPIO 39  (ймовірно)
  SCK:  GPIO 40  (ймовірно)
  CS:   GPIO 12  (dedicated)
```

> ⚠️ **Відкрите питання OQ-01:** LDC1101 Click Board підключений через SPI (MIKROE-3240).
> Якщо SD і LDC1101 використовують **один SPI bus** — потрібне multiplexing через CS pins.
> Якщо різні — немає конфлікту. Потребує підтвердження на залізі.
...
Storage bookkeeping (meas_count, meas_head, log_rotate) ≈ 5 entries
...
#### B-06 — SPI Bus Conflict між LDC1101 та SD [MEDIUM, потребує верифікації]
Якщо LDC1101 та SD card на одному SPI bus — одночасний доступ неможливий.
Logger SDTransport (async FreeRTOS task) може намагатись писати на SD під час SPI transfer LDC1101.
**Fix:** або розведення на різні SPI bus, або mutual exclusion mutex між storage та sensor layers.
...
│   └── "proto_id"  : String  # Protocol ID під час калібрування (наприклад "4dist_v1")
│
...
│   ├── "meas_head" : UInt8   # Ring buffer head (0 .. RING_SIZE-1)
│   ├── "meas_size" : UInt8   # Current ring buffer size (const після init = 50)
...
public:
    ...
    uint8_t  getMeasHead() const;
    void     incrementMeasCount();    // атомарно count++ та head = (head+1) % RING_SIZE
...
│
├── measurements/                   # Ring buffer вимірів (≤50 × 6 KB = 300 KB)
...
└── logs/                           # Rotating logs (~600–900 KB)
    ├── log.0.txt                   # Поточний (max 300 KB)
    ├── log.1.txt                   # Попередній
    └── log.2.txt                   # Архів (найстаріший, перезаписується при ротації)
```
...
Кожен `m_XXX.json` (де XXX = `meas_count % 50`):

```json
{
  "schema_ver": 1,
  "algo_ver": 1,
  "protocol_id": "4dist_v1",
  "protocol_ver": "1.0",
...
─────────────────────────────────────────────
Вимірювання       | ✅ Full  | ✅ Full (ring buffer 50)
...
Архів вимірів     | ✅ Full  | ❌ Тільки останні 50
Логи (архів)      | ✅ Full  | ⚠️ Тільки 3 файли ротації в LittleFS
Fingerprint DB    | ✅ Full  | ⚠️ Стара версія з cache/ (якщо є)
```

### 9.4 SD + LDC1101 SPI Conflict Resolution

**Варіант A: Shared SPI Bus (один SPIClass)**
```cpp
// Обидва на одному VSPI bus, різні CS
SPIClass spi(VSPI);
spi.begin(SCK, MISO, MOSI);
LDC1101 ldc(spi, LDC_CS_PIN);
SD.begin(SD_CS_PIN, spi);

// Mutex для захисту
SemaphoreHandle_t spi_mutex = xSemaphoreCreateMutex();
```
Плюси: простіше, менше pins. Мінуси: якщо Logger SDTransport (async task) намагається писати під час LDC measurement → deadlock ризик без proper mutex.

**Варіант B: Separate SPI Bus**
```cpp
// LDC1101 на SPI2 (HSPI), SD на SPI3 (VSPI)
SPIClass spi_ldc(HSPI); // pins для LDC
SPIClass spi_sd(VSPI);  // pins для SD
// Незалежні, без конфлікту
```
Плюси: нема конфліктів, паралельний доступ. Мінуси: більше pin використання (якщо є вільні на Cardputer-Adv — потребує перевірки).

> **Рекомендація: Варіант A з mutex** для першої версії (менше змін у hardware wiring).
> Варіант B — якщо профілювання покаже що SD mutex є bottleneck.

---

## 10. Критичні питання для обговорення

### OQ-01 — LDC1101 та SD на одному SPI? [Hardware verification needed]
Потребує перевірки схеми Cardputer-Adv та Click Board підключення. Відповідь визначає SPI architecture.

**Варіант обговорення:**
- A) Спільний SPI + mutex → проблема з async Logger SDTransport
- B) Два SPI bus → потребує вільних GPIO

### OQ-02 — Одна чи дві LittleFS partitions? [Архітектурний вибір]
Залежить від пріоритету: простота vs захист даних.

**Варіант обговорення:**
- A) Одна partition (Option A) + backup script → простіше, але `uploadfs` ризикований
- B) Дві partitions (Option B) → складніше але правильно; потребує custom PIO script ← рекомендація
- C) Прийняти що виміри губляться при uploadfs → так роблять більшість прошивок, але не для CoinTrace

### OQ-03 — Ring buffer size: скільки вимірів зберігати? [Product decision]
...
### OQ-04 — Де зберігати plugin config.json? [Архітектурний вибір]
Plugin Architecture визначає `data/config.json` як джерело плагін конфігурації.

**Варіант обговорення:**
- A) LittleFS `/sys/config/device.json` → оновлюється тільки з uploadfs ← рекомендація
- B) LittleFS `/data/config/device.json` → не губиться при uploadfs, але складніше оновити
- C) NVS у namespace "plugins" → атомарно, але не файл і важко редагувати вручну

### OQ-05 — Чи потрібен NVS encryption (AES-XTS)? [Security decision]
...
### OQ-06 — Де зберігати "session ID" вимірювань? [Traceability]
При community DB submission потрібно прив'язати кілька вимірів однієї сесії.

**Варіант обговорення:**
- A) UUID генерується при старті, зберігається в NVS до наступного старту
- B) UUID = `device_id + boot_count` (детерміновано)
...
**Статус:** 🔄 На обговоренні (потребує підтвердження OQ-02)
...
**Рекомендація:** Варіант B — відповідає `IPlugin` / `ILogTransport` стилю проекту.

---

### ADR-ST-006: Ring Buffer 50 вимірів у LittleFS_data

**Статус:** 🔄 На обговоренні (OQ-03)

**Контекст:** Скільки вимірів зберігати on-device без SD.

**Рішення:** 50 файлів `m_000.json` .. `m_049.json`. Head pointer у NVS.

**Аргументи:**
- 50 × 5 KB = 250 KB — комфортно в 1.75 MB partition
- 50 вимірів при ~3 хв/вимір = ~2.5 год автономної роботи
- Файловий ring (не in-memory) → виміри виживають при reboot та crash

**Наслідки:**
- Head pointer `meas_head` у NVS атомарно оновлюється ПІСЛЯ успішного write файлу
- При crash під час write → файл m_XXX.json може бути порожнім або truncated → validate при читанні
...
Logger dispatch chain:
  ...
  LittleFSTransport → /data/logs/log.0.txt (async queue, power-fail safe, NEW)
...
- Background task дренує queue, пише в `log.0.txt`
- При розмірі `log.0.txt` > 300 KB: ротація (rename `.0`→`.1`, `.1`→`.2`, delete `.2`)
- При підключенні SD та ротації: копіює `log.2.txt` → `SD:/CoinTrace/logs/archive/<date>.txt`
...
|---|---|
| `GET /api/v1/status` | NVS read: heap, version, bat, wifi, ble |
| `GET /api/v1/measure/{id}` | LittleFS read: `/data/measurements/m_XXX.json`, TTL 60s |
...
| `src/storage/MeasurementStore.h/.cpp` | Ring buffer 50 × m_XXX.json, versioned format |
...
| Артефакт | Що робить |
|---|---|
| `src/storage/SDCardManager.h/.cpp` | Optional SD mount, graceful failure, SPI mutex |
...
|---|---|---|---|
| **OQ-01** | LDC1101 та SD на одному SPI bus? | SPI mutex architecture | Перевірити схему фізично |
| **OQ-02** | Одна чи дві LittleFS partitions? | uploadfs safety | Архітектурне рішення |
| **OQ-03** | Ring buffer size (50 вимірів достатньо?) | UX, storage allocation | Product decision |
| **OQ-04** | Де plugin config.json? | Boot dependency | Архітектурне рішення |
| **OQ-05** | NVS flash encryption до v1 чи після? | Security, one-way decision! | Security decision |
| **OQ-06** | Session ID для вимірів — як генерувати? | Community DB трасованість | Algorithm decision |
| **OQ-07** | LittleFS log rotation — окремий transport чи periodic dump? | Logger architecture | Implementation decision |

---

*Версія 0.1.0 — Draft для обговорення*
*Наступний крок: вирішити OQ-01..OQ-07, зафіксувати ADR-ST-002 (partition option), реалізувати P-1 (bug fix + partition table)*
