...
**Версія аудиту:** 1.2.0  
**Дата:** 13 березня 2026 (оновлено: хвиля 4 — LA-2 skin depth, ADR-RESP-001, TC1/TC2; хвиля 5 — 14 березня 2026 — ADR-COIN-001: детекція монети, `CoinState`, `updateCoinState()`; SYS-10 CLOSED)  
...
**Знайдено 24 знахідки (v1.1.0); v1.2.0 закрило 6; v1.3.0 закрило 1 (ADR-COIN-001): **0 CRITICAL · 4 HIGH · 7 MEDIUM · 4 LOW · 2 INFO** відкритих
...
При f = 1 MHz (⚠️ LA-2: попередні значення цієї таблиці були помилково розраховані
при f = 10 MHz і позначені як 1 MHz — похибка −√10 ≈ 3.16×; виправлено в хвилі 4):

Метал         σ [MS/m]   μr      δ [µm]   Висновок
──────────────────────────────────────────────────────────────────
Срібло Ag999  63.0       1.0     63.4     Глибоке проникнення при 1 MHz
Мідь Cu       58.5       1.0     65.8     Схоже на Ag (critical ambiguity!)
Золото Au     44.2       1.0     75.7     Менша провідність → більший δ
Алюміній Al   35.7       1.0     84.2     Помітно відрізняється від Ag
Нікель Ni     14.3      600.0     5.4     Дуже малий δ (феромагнетик!)
Сталь 304     1.37      750.0    15.7     Мала σ, але великий μr → малий δ
Вольфрам W    18.9       1.0    115.7     Велика маса → deepfake Au/Ag

⚠️ MIKROE-3240 реальний fSENSOR ≈ 200–500 kHz (§3.7), не 1 MHz!
   При 300 kHz δ × 1.826: δ(Ag)≈116 µm, δ(Ni)≈9.9 µm, δ(W)≈211 µm.
   Повна таблиця при 200–500 kHz: LDC1101_AUDIT_v1.0.0.md §SIM-2.

Ключовий висновок: δ(Ag) ≈ δ(Cu) при 1 MHz (63.4 vs 65.8 µm, різниця ~2.4 µm) — система
МОЖЕ плутати Ag та Cu якщо покладатись лише на RP! Диференціація вимагає
L-канал (μr Ag = μr Cu = 1) або більш складного аналізу профілю кривої RP(d).
```

**⚠️ КРИТИЧНА ЗНАХІДКА SIM-1:** При f = 1 MHz skin depth срібла та міді відрізняється лише на ~2.4 µm (63.4 vs 65.8 µm; ⚠️ LA-2: попереднє значення ~0.8 µm було розраховане при 10 MHz). Монети однакового розміру зі срібла та міді (однакова геометрія, однаковий діаметр) дадуть дуже близькі значення RP через близькість провідностей (63 vs 58.5 MS/m = 7.6% різниця). Диференціація можлива лише через:
...
⚠️ ВИСНОВОК: fSENSOR MIKROE-3240 оцінюється як 200–500 kHz, а НЕ 1–2 MHz!
   ...
   - При 300 kHz: δ(Ag) ≈ 63.4 µm × √(1MHz/300kHz) ≈ 116 µm (1.8× глибше)
...
**GAP-1: Coin detection trigger** ✅ **ВИРІШЕНО — ADR-COIN-001**

```
Вирішено в LDC1101_ARCHITECTURE.md v1.1.0 (14 березня 2026).

Обраний механізм: Threshold-based RP (Варіант A) — dual-threshold hysteresis.
  DETECT:  rpRaw < calibrationRpBaseline × 0.85  (N=5 підряд → COIN_PRESENT)
  RELEASE: rpRaw > calibrationRpBaseline × 0.92  (M=3 підряд → COIN_REMOVED)
  CoinState enum: IDLE_NO_COIN | COIN_PRESENT | COIN_REMOVED

Параметри (verifiable при R5): coin_detect_threshold, coin_release_threshold,
  detect_debounce_n, release_debounce_m — всі в ldc1101.json.

Варіанти B/C/D відхилено (B: крос-плагінна залежність; C: погана UX; D: немає сенсора).
ЗАКРИТО: SYS-10 → CLOSED. R-B2 виконано.
```
...
| **Coin Detection Trigger** | ✅ ВИРІШЕНО | **ADR-COIN-001** в LDC1101_ARCHITECTURE.md v1.1.0: dual-threshold hysteresis (DETECT×0.85 / RELEASE×0.92), `CoinState` enum, `updateCoinState()` |
...
|--|--|--|--|--|
| **SYS-1** | ✅ CLOSED | LDC1101 | `readRP()` — MSB читалось ДО LSB; bug підтверджений в MikroE reference SDK | **FIXED PA3-1:** PLUGIN_DIAGNOSTICS.md burst read з CS LOW |
| **SYS-1b** | ✅ CLOSED | LDC1101 | RP_SET mismatch: 0x07 (arch doc) vs 0x26 (MikroE SDK) | **ADR-LDC-001** в LDC1101_ARCHITECTURE.md: 0x26 як default |
| **SYS-2** | ✅ CLOSED | Plugin | IInputPlugin API конфлікт `pollEvent()` vs `hasEvent()`/`getEvent()` | **FIXED PA3-5:** INTERFACES_EXTENDED + CONTRACT узгоджені |
| **SYS-3** | ✅ CLOSED | Logger | `usePsram=true` у `src/main.cpp` (Bug B-01) | **FIXED LA-1:** `usePsram=false` в main.cpp |
| **SYS-4** | ✅ CLOSED | Display | Display (ST7789V2) на VSPI без spiMutex — race condition | **ADR-SPI-001** в LDC1101_ARCHITECTURE.md: Strategy A only |
| **SYS-5** | ✅ CLOSED | LDC1101 | TC1/TC2 не задокументовані в архітектурі | **FIXED v1.2.0:** TC1=0x1F, TC2=0x3F + DIG_CFG=0xD4 в LDC1101_ARCHITECTURE.md |
| **SYS-6** | 🟠 HIGH | LDC1101 | Температурна компенсація відсутня (±11% per 30°C) | Новий |
| **SYS-7** | 🟠 HIGH | LDC1101 | fSENSOR оцінено як 200–500 kHz (не 1–2 MHz); C_SENSOR і точна геометрія котушки потребують вимірювання | Оцінка задокументована в LDC1101_ARCHITECTURE.md; вимірювання на реальному пристрої потрібне |
...
| **SYS-10** | ✅ CLOSED | System | Coin detection trigger — відсутній у всій архітектурі | **ADR-COIN-001** в LDC1101_ARCHITECTURE.md v1.1.0: dual-threshold hysteresis, `CoinState` enum, `updateCoinState()` |
...
| Coin Detection (trigger mechanism) | ✅ 85% | **ADR-COIN-001** в LDC1101_ARCHITECTURE.md v1.1.0; impl + hardware verify при R5 |
...
Готовність до публічного beta (community DB):                  ⚠️ ~60%
  Блокери: R-01 validation set, SYS-4 (display mutex), SYS-17 (OTA security), UI Architecture
...
~~**R-B2** — Написати ADR "Coin Detection Trigger" в LDC1101_ARCHITECTURE.md~~ ✅ **ВИКОНАНО** (ADR-COIN-001, LDC1101_ARCHITECTURE.md v1.1.0, 14 березня 2026)
