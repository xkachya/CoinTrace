...
# не нормалізовані координати centroid з `index.json` (де dRp1_n = dRp1/800).  # FDB-03: dRp1_MAX 600→800 Ohm (v1.5.0)
BOUNDS = {
...
> Firmware нормалізує новий вимір аналогічно **перед** порівнянням з `index.json`. Порушення цього правила призведе до того, що `dRp1` (0–800 range) домінує над `k1`, `k2` (0–1 range) і пошук стане некоректним.
